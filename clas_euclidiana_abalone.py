...
abalone_test_predictions = euclidean_classifier(abalone_train, abalone_test)

# Calcula la precisión para el conjunto de prueba Hold Out
abalone_test_accuracy = calculate_accuracy(abalone_test.iloc[:, 0], abalone_test_predictions)
print(f'Hold Out Precision: {abalone_test_accuracy}')
...
abalone_fold_accuracies = []
for i in range(len(abalone_folds)):
    test_fold = abalone_folds[i]
    train_fold = pd.concat(abalone_folds[:i] + abalone_folds[i+1:], ignore_index=True)
    fold_predictions = euclidean_classifier(train_fold, test_fold)
    fold_accuracy = calculate_accuracy(test_fold.iloc[:, 0], fold_predictions)
    abalone_fold_accuracies.append(fold_accuracy)
print(f'Cross-Validation Precision: {abalone_fold_accuracies}')
print(f'Promedio Cross-Validation Precision: {np.mean(abalone_fold_accuracies)}')
